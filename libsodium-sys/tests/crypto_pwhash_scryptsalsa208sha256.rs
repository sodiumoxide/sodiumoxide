...
fn test_crypto_pwhash_scryptsalsa208sha256_strprefix() {
    unsafe {
         ...
         let b = std::ffi::CStr::from_bytes_with_nul(crypto_pwhash_scryptsalsa208sha256_STRPREFIX).unwrap();
        assert_eq!(s, b);
    }
}
...
fn test_crypto_pwhash_scryptsalsa208sha256_str() {
    ...
    let mut hashed_password =
        [0; crypto_pwhash_scryptsalsa208sha256_STRBYTES as usize];
    let ret_hash = unsafe {
    ...
    let ret_verify = unsafe {
        crypto_pwhash_scryptsalsa208sha256_str_verify(
            hashed_password.as_ptr() as *const i8,
            password.as_ptr() as *const i8,
            password.len() as c_ulonglong)
    };
    assert!(ret_verify == 0);
