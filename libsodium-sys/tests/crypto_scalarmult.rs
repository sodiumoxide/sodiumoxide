...
fn test_crypto_scalarmult_bytes() {
    assert_eq!(unsafe { crypto_scalarmult_bytes() },
               crypto_scalarmult_BYTES as usize);
}

#[test]
fn test_crypto_scalarmult_scalarbytes() {
    assert_eq!(unsafe { crypto_scalarmult_scalarbytes() },
...
fn test_crypto_scalarmult_primitive() {
    unsafe {
        ...
        let b = std::ffi::CStr::from_bytes_with_nul(crypto_scalarmult_PRIMITIVE).unwrap();
        assert_eq!(s, b);
    }
}
