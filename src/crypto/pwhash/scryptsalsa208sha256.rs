...
pub const STRPREFIX: &'static str = "$7$"; // ffi::crypto_pwhash_scryptsalsa208sha256_STRPREFIX
...
pub fn pwhash(passwd: &[u8], OpsLimit(opslimit): OpsLimit,
    ...
    if unsafe {
        ...
        ffi::crypto_pwhash_scryptsalsa208sha256_str(str_.as_mut_ptr() as (*mut i8),
                                                    passwd.as_ptr() as (*const i8),
                                                    passwd.len() as c_ulonglong,
                                                    opslimit as c_ulonglong,
                                                    memlimit)
...
pub fn pwhash_verify(&HashedPassword(ref str_): &HashedPassword,
                     passwd: &[u8]) -> bool {
    unsafe {
        ffi::crypto_pwhash_scryptsalsa208sha256_str_verify(str_.as_ptr() as (*const i8),
                                                           passwd.as_ptr() as (*const i8),
                                                           passwd.len() as c_ulonglong)
            == 0
    }
