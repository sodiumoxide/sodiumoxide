...
pub const HEADERBYTES: usize = $headerbytes;

/// Number of added bytes
pub const ABYTES: usize = $abytes;
...
pub fn init_push(key: &Key) -> (State, [u8; HEADERBYTES]) {
    unsafe {
        ...
        $init_push_name(&mut state, &mut header, &key.0);
        (State(state), header)
    }
}
...
pub fn init_pull(header: &[u8; HEADERBYTES], key: &Key) -> Result<State, ()> {
    unsafe {
        let mut state: $state_name = mem::uninitialized();
        if $init_pull_name(&mut state, header, &key.0) != 0 {
            return Err(());
        }
        Ok(State(state))
    }
