...
pub fn verify(sm: &[u8],
    ...
    let mut mlen = 0;
    let ret = unsafe {
        ffi::crypto_sign_ed25519_open(m.as_mut_ptr(),
                                      &mut mlen,
                                      sm.as_ptr(),
...
pub fn verify_detached(sig: &Signature,
                       ...
                       pk: &PublicKey) -> bool {
    let ret = unsafe {
        ffi::crypto_sign_ed25519_verify_detached(sig.0.as_ptr(),
                                                 m.as_ptr(),
                                                 m.len() as c_ulonglong,
                                                 pk.0.as_ptr())
    };
...
impl State {
    ...
    pub fn verify(
        ...
        let ret = unsafe { ffi::crypto_sign_ed25519ph_final_verify(&mut self.0, sig.as_mut_ptr(), pk.as_ptr()) };
        ret == 0
    }
}
