...
#[link_args = "-lsodium"]
extern {
    fn crypto_stream_salsa208(c: *mut u8,
                              clen: c_ulonglong,
                              n: *u8,
                              k: *u8) -> c_int;
    fn crypto_stream_salsa208_xor(c: *mut u8,
                                  m: *u8,
                                  mlen: c_ulonglong,
                                  n: *u8,
                                  k: *u8) -> c_int;
...
pub fn stream(len: uint, n: &Nonce, k: &Key) -> ~[u8] {
    unsafe {
        ...
        crypto_stream_salsa208(to_mut_ptr(c), 
...
pub fn stream_xor(m: &[u8], n: &Nonce, k: &Key) -> ~[u8] {
    let (c, _) = do marshal(m, 0, 0) |dst, src, len| {
        unsafe {
            crypto_stream_salsa208_xor(dst, src, len, to_ptr(**n), to_ptr(**k))
        }
    };
    c
}
...
pub fn stream_xor_inplace(m: &mut [u8], n: &Nonce, k: &Key) {
    unsafe {
        crypto_stream_salsa208_xor(to_mut_ptr(m), 
                                   to_ptr(m), 
